...
if (os.name=="nt"):    
    ...
    API=input("Enter your API key: ")
print("\nauto refresh mode is set to on")
city=input("\nEnter city name in the format (city,country)\nAns:") #input by the user
if (os.name=="nt"):
    os.system("cls")
else:
    os.system("clear")
if (city.lower()=="delhi"):
    city="delhi,india"
def autorefresh(city,api,t): #function for autorefreshing the weather forecast function
    while True:
...
def weather_forcast(city,Api):
    ...
    if (fetch_data.status_code ==200): #if data is fetched successfully
        ...
        weather_forcast(input("\nEnter city name in the format (city,country)\nAns:"),input("\nEnter API KEY:")) #recall the function with new api key and new city name 
    autorefresh(city,Api,datetime.now().time().strftime("%S")) #calling the autorefresh function
weather_forcast(city,API) #main function call